#!/bin/bash
# Weekly security-career coach runner — headless `claude -p`.
# Invoked by launchd (com.example.secondbrain-coach) every Friday, or manually.
set -euo pipefail

ROOT="${SECONDBRAIN_HOME:-$HOME/.secondbrain}"
VAULT="${SECONDBRAIN_VAULT:-$HOME/SecondBrain}"
PROMPT_FILE="$ROOT/weekly-coach-prompt.md"
LOCKDIR="$ROOT/.lock-coach"
STOP="$ROOT/STOP"
LOG="$ROOT/logs/coach-$(date +%Y%m%d-%H%M%S).log"
...
mkdir -p "$ROOT/logs"
exec > >(tee -a "$LOG") 2>&1
echo "=== secondbrain-coach run $(date) (model=$MODEL, budget=\$$BUDGET_USD) ==="
...
echo "=== secondbrain-coach run complete $(date) ==="
